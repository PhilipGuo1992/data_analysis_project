...
tweet_data = sqlContext.read.format('com.databricks.spark.csv').options(header='true', inferschema='true').load('real_time_Tweets.csv')
model.transform(tweet_data) \
    .select("SentimentText", "created_time", "probability", "prediction") \
    .save('real_tweets.csv', 'com.databricks.spark.csv')
